...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useEvolutionStellarTokenV = true // Default to use V = 27 or 28, the legacy EvolutionStellarToken format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useEvolutionStellarTokenV, err
	}
	...
	case ApplicationClique.Mime:
		// Clique is the EvolutionStellarToken PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useEvolutionStellarTokenV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
		...
		useEvolutionStellarTokenV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: cliqueRlp, Messages: messages, Hash: sighash}
	default: // also case TextPlain.Mime:
		// Calculates an EvolutionStellarToken ECDSA signature for:
		// hash = keccak256("\x19${byteVersion}EvolutionStellarToken Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useEvolutionStellarTokenV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
	...
	return req, useEvolutionStellarTokenV, nil
}
