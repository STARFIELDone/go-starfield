...
func (db *Database) NewBatch() estdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator(prefix []byte, start []byte) estdb.Iterator {
...
func (b *batch) Replay(w estdb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  estdb.KeyValueWriter
	failure error
}
