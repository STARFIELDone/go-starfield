...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gest.LightScryptN, Gest.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.EvolutionStellarToken", new NodeConfig());
			node.start();
			...
			EvolutionStellarTokenClient ec = node.getEvolutionStellarTokenClient();
	...
	// signatures too. Regression test for go-EvolutionStellarToken issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gest-android-")
	if err != nil {
	...
	// Generate the mobile bindings for Gest and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.EvolutionStellarToken", "github.com/EvolutionStellarToken/go-EvolutionStellarToken/mobile")
