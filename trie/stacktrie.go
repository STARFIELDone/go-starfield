...
func stackTrieFromPool(db estdb.KeyValueStore) *StackTrie {
...
type StackTrie struct {
	...
	db estdb.KeyValueStore // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db estdb.KeyValueStore) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func newLeaf(ko int, key, val []byte, db estdb.KeyValueStore) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db estdb.KeyValueStore) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)
