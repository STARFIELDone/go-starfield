...
type Sync struct {
	database estdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database estdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
// both est/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw estdb.Batch) error {
