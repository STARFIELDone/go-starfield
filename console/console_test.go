...
type tester struct {
	workspace string
	stack     *node.Node
	EvolutionStellarToken  *est.EvolutionStellarToken
	console   *Console
...
func newTester(t *testing.T, confOverride func(*est.Config)) *tester {
	...
	// Create a networkless protocol stack and start an EvolutionStellarToken service within
	...
	ethBackend, err := est.New(stack, ethConf)
	if err != nil {
		t.Fatalf("failed to register EvolutionStellarToken protocol: %v", err)
	}
	...
	return &tester{
		workspace: workspace,
		stack:     stack,
		EvolutionStellarToken:  ethBackend,
		console:   console,
		input:     prompter,
		output:    printer,
	}
