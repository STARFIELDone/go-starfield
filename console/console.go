...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/internal/jsre"
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/internal/jsre/deps"
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute EvolutionStellarToken requests through
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"est": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines est.getBalance(est.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gest instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gest JavaScript console!\n\n"

	// Print some generic Gest metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + est.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + est.blockNumber + " (" + new Date(1000 * est.getBlock(est.blockNumber).timestamp) + ")\n";
		try {
