...
// Package eststats implements the network stats reporting service.
package eststats
...
// backend encompasses the bare-minimum functionality needed for eststats reporting
...
// Service implements an EvolutionStellarToken netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(eststats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["est"]; info != nil {
		network = fmt.Sprintf("%d", info.(*est.NodeInfo).Network)
		protocol = fmt.Sprintf("est/%d", est.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the eststats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to eststats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to eststats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to eststats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to eststats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to eststats")
