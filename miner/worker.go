...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, est Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *worker {
	worker := &worker{
		config:             config,
		...
		est:                est,
		mux:                mux,
		chain:              est.BlockChain(),
		isLocalBlock:       isLocalBlock,
		localUncles:        make(map[common.Hash]*types.Block),
		remoteUncles:       make(map[common.Hash]*types.Block),
		unconfirmed:        newUnconfirmedBlocks(est.BlockChain(), miningLogAtDepth),
	...
	worker.txsSub = est.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = est.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = est.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	pending, err := w.est.TxPool().Pending()
	if err != nil {
	...
	for _, account := range w.est.TxPool().Locals() {
...
// totalFees computes total consumed fees in est. Block transactions and receipts have to have the same order.
