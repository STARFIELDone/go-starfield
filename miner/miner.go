...
// Package miner implements EvolutionStellarToken block creation and mining.
package miner
...
func New(est Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		est:     est,
		mux:     mux,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, est, mux, isLocalBlock, true),
	}
	go miner.update()
