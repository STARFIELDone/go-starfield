...
func (p *peerCommons) Info() *est.PeerInfo {
	return &est.PeerInfo{
		Version:    p.version,
		Difficulty: p.Td(),
	...
	return p.handshake(td, head, headNum, genesis, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local EvolutionStellarToken node is running in archive mode, advertise ourselves we have
...
// participating in the Light EvolutionStellarToken sub-protocol.
