...
// Package les implements the Light EvolutionStellarToken Subprotocol.
package les
...
import (
	...
	lpc "github.com/EvolutionStellarToken/go-EvolutionStellarToken/les/lespay/client"
...
type LightEvolutionStellarToken struct {
	lesCommons
...
func New(stack *node.Node, config *est.Config) (*LightEvolutionStellarToken, error) {
	chainDb, err := stack.OpenDatabase("lightchaindata", config.DatabaseCache, config.DatabaseHandles, "est/db/chaindata/")
	if err != nil {
		return nil, err
	}
	lespayDb, err := stack.OpenDatabase("lespay", 0, 0, "est/db/lespay")
	if err != nil {
		return nil, err
	}
	...
	leth := &LightEvolutionStellarToken{
		...
		engine:         est.CreateConsensusEngine(stack, chainConfig, &config.Ethash, nil, false, chainDb),
		bloomRequests:  make(chan chan *bloombits.Retrieval),
		bloomIndexer:   est.NewBloomIndexer(chainDb, params.BloomBitsBlocksClient, params.HelperTrieConfirmations),
...
// APIs returns the collection of RPC services the EvolutionStellarToken package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightEvolutionStellarToken) APIs() []rpc.API {
...
func (s *LightEvolutionStellarToken) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightEvolutionStellarToken) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightEvolutionStellarToken) TxPool() *light.TxPool              { return s.txPool }
func (s *LightEvolutionStellarToken) Engine() consensus.Engine           { return s.engine }
func (s *LightEvolutionStellarToken) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightEvolutionStellarToken) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightEvolutionStellarToken) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols returns all the currently configured network protocols to start.
func (s *LightEvolutionStellarToken) Protocols() []p2p.Protocol {
...
// light EvolutionStellarToken protocol implementation.
func (s *LightEvolutionStellarToken) Start() error {
...
func (s *LightEvolutionStellarToken) Stop() error {
	close(s.closeCh)
	s.serverPool.stop()
	s.valueTracker.Stop()
	s.peers.close()
	...
	s.chainDb.Close()
	s.wg.Wait()
	log.Info("Light EvolutionStellarToken stopped")
	return nil
}
