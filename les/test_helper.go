...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/accounts/abi/bind/backends"
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/contracts/checkpointoracle/contract"
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/les/checkpointoracle"
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/les/flowcontrol"
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/p2p/nodestate"
...
func testIndexers(db estdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
	var indexers [3]*core.ChainIndexer
	indexers[0] = light.NewChtIndexer(db, odr, config.ChtSize, config.ChtConfirms, disablePruning)
	indexers[1] = est.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db estdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
	...
	client := &LightEvolutionStellarToken{
		lesCommons: lesCommons{
			genesis:     genesis.Hash(),
			config:      &est.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db estdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
