...
import (
	...
	lps "github.com/EvolutionStellarToken/go-EvolutionStellarToken/les/lespay/server"
...
const (
	...
	ethVersion        = 63              // equivalent est version for the downloader
...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb estdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light EvolutionStellarToken peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), h.server); err != nil {
		p.Log().Debug("Light EvolutionStellarToken handshake failed", "err", err)
		return err
	}
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light EvolutionStellarToken peer rejected", "err", errFullClientPool)
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light EvolutionStellarToken response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light EvolutionStellarToken message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light EvolutionStellarToken message arrived", "code", msg.Code, "bytes", msg.Size)
