...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/p2p/simulations"
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/p2p/simulations/adapters"
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	...
	EvolutionStellarToken, err := est.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, EvolutionStellarToken, &config)
	if err != nil {
		return nil, err
	}
	return EvolutionStellarToken, nil
}
