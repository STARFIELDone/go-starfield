...
func (est *LightEvolutionStellarToken) startBloomHandlers(sectionSize uint64) {
		...
		go func() {
			...
			for {
				...
				case request := <-est.bloomRequests:
					...
					compVectors, err := light.GetBloomBits(task.Context, est.odr, task.Bit, task.Sections)
					if err == nil {
