...
// the est.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb estdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
//
// Note, we rely on the underlying est/fetcher to retrieve and validate the
// response, so that we have to obey the rule of est/fetcher which only accepts
