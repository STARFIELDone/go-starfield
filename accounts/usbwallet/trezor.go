...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/accounts/usbwallet/trezor"
...
// or whether the EvolutionStellarToken app was not started on it.
...
// EvolutionStellarToken address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.EvolutionStellarTokenAddress)
	if _, err := w.trezorExchange(&trezor.EvolutionStellarTokenGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.EvolutionStellarTokenSignTx{
	...
	response := new(trezor.EvolutionStellarTokenTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EvolutionStellarTokenTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
