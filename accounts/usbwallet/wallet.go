...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the EvolutionStellarToken
...
type wallet struct {
	...
	deriveChain     EvolutionStellarToken.ChainStateReader // Blockchain state reader to discover used account with
	...
	for errc == nil && err == nil {
		...
		for i := 0; i < len(nextAddrs); i++ {
			for empty := false; !empty; {
				// Retrieve the next derived EvolutionStellarToken account
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain EvolutionStellarToken.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
