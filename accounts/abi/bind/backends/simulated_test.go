...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != EvolutionStellarToken.NotFound {
		t.Fatalf("err should be `EvolutionStellarToken.NotFound` but received %v", err)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	var cases = []struct {
		name        string
		message     EvolutionStellarToken.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"Revert", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"PureRevert", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"OOG", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      100000,
			GasPrice: big.NewInt(0),
		...
		{"Assert", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      100000,
			GasPrice: big.NewInt(0),
		...
		{"Valid", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      100000,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", EvolutionStellarToken.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, EvolutionStellarToken.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, EvolutionStellarToken.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, EvolutionStellarToken.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, EvolutionStellarToken.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
