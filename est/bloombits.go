...
const (
	// bloomServiceThreads is the number of goroutines used globally by an EvolutionStellarToken
...
func (est *EvolutionStellarToken) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-est.closeBloomHandler:
					return

				case request := <-est.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(est.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(est.chainDb, task.Bit, section, head); err == nil {
...
// for the EvolutionStellarToken header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size    uint64               // section size to generate bloombits for
	db      estdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db estdb.Database, size, confirms uint64) *core.ChainIndexer {
