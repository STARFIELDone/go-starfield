...
// PeerInfo represents a short summary of the EvolutionStellarToken sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // EvolutionStellarToken protocol version negotiated
...
//
// This method is legacy support for initial transaction exchange in est/64 and
// prior. For est/65 and higher use SendPooledTransactionHashes.
...
// Handshake executes the est protocol handshake, negotiating version number,
...
func (p *peer) Handshake(network uint64, td *big.Int, head common.Hash, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter) error {
	...
	go func() {
		...
		default:
			panic(fmt.Sprintf("unsupported est protocol version: %d", p.version))
		}
	}()
	go func() {
		switch {
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("est/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the EvolutionStellarToken sub-protocol.
type peerSet struct {
