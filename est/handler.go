...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/est/fetcher"
...
// NewProtocolManager returns a new EvolutionStellarToken sub protocol manager. The EvolutionStellarToken sub protocol manages peers capable
// with the EvolutionStellarToken network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb estdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing EvolutionStellarToken peer", "peer", id)

	// Unregister the peer from the downloader and EvolutionStellarToken peer set
...
func (pm *ProtocolManager) Start(maxPeers int) {
	...
	go pm.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with est/64.
}
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("EvolutionStellarToken protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an est peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("EvolutionStellarToken peer connected", "name", p.Name())

	// Execute the EvolutionStellarToken handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkID, pm.forkFilter); err != nil {
		p.Log().Debug("EvolutionStellarToken handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p, pm.removePeer); err != nil {
		p.Log().Error("EvolutionStellarToken peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("EvolutionStellarToken message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the EvolutionStellarToken sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // EvolutionStellarToken network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
