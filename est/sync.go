...
func (pm *ProtocolManager) syncTransactions(p *peer) {
	...
	// The est/65 protocol introduces proper transaction announcements, so instead
...
func (pm *ProtocolManager) txsyncLoop64() {
	...
	send := func(s *txsync) {
		if s.p.version >= eth65 {
			panic("initial transaction syncer running on est/65+")
		}
...
func (pm *ProtocolManager) doSync(op *chainSyncOp) error {
	if op.mode == downloader.FastSync {
		...
		// txlookup limit.
		// The main concern here is: during the fast sync Gest won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next fast sync(e.g. user kill Gest manually and
		// restart) then it will be hard for Gest to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the fast sync. If changed, Gest
