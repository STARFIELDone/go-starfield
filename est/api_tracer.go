// Copyright 2017 The go-EvolutionStellarToken Authors
...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/est/tracers"
...
func (api *PrivateDebugAPI) TraceChain(ctx context.Context, start, end rpc.BlockNumber, config *TraceConfig) (*rpc.Subscription, error) {
	...
	case rpc.PendingBlockNumber:
		from = api.est.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		from = api.est.blockchain.CurrentBlock()
	default:
		from = api.est.blockchain.GetBlockByNumber(uint64(start))
	}
	switch end {
	case rpc.PendingBlockNumber:
		to = api.est.miner.PendingBlock()
	case rpc.LatestBlockNumber:
		to = api.est.blockchain.CurrentBlock()
	default:
		to = api.est.blockchain.GetBlockByNumber(uint64(end))
	}
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	database := state.NewDatabaseWithCache(api.est.ChainDb(), 16, "") // Chain tracing will probably start at genesis

	if number := start.NumberU64(); number > 0 {
		start = api.est.blockchain.GetBlock(start.ParentHash(), start.NumberU64()-1)
		if start == nil {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				signer := types.MakeSigner(api.est.blockchain.Config(), task.block.Number())
				blockCtx := core.NewEVMBlockContext(task.block.Header(), api.est.blockchain, nil)
				...
				for i, tx := range task.block.Transactions() {
					...
					task.statedb.Finalise(api.est.blockchain.Config().IsEIP158(task.block.Number()))
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			block := api.est.blockchain.GetBlockByNumber(number)
			if block == nil {
			...
			_, _, _, err := api.est.blockchain.Processor().Process(block, statedb, vm.Config{})
			if err != nil {
				failed = err
				break
			}
			// Finalize the state so any modifications are written to the trie
			root, err := statedb.Commit(api.est.blockchain.Config().IsEIP158(block.Number()))
			if err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) TraceBlockByNumber(ctx context.Context, number rpc.BlockNumber, config *TraceConfig) ([]*txTraceResult, error) {
	...
	switch number {
	case rpc.PendingBlockNumber:
		block = api.est.miner.PendingBlock()
	case rpc.LatestBlockNumber:
...
func (api *PrivateDebugAPI) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if err := api.est.engine.VerifyHeader(api.est.blockchain, block.Header(), true); err != nil {
		return nil, err
	}
	parent := api.est.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		signer = types.MakeSigner(api.est.blockchain.Config(), block.Number())
	...
	blockCtx := core.NewEVMBlockContext(block.Header(), api.est.blockchain, nil)
	for th := 0; th < threads; th++ {
		pend.Add(1)
		go func() {
			defer pend.Done()
	...
	for i, tx := range txs {
		...
		vmenv := vm.NewEVM(blockCtx, txContext, statedb, api.est.blockchain.Config(), vm.Config{})
...
func (api *PrivateDebugAPI) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	var (
		signer      = types.MakeSigner(api.est.blockchain.Config(), block.Number())
		dumps       []string
		chainConfig = api.est.blockchain.Config()
		vmctx       = core.NewEVMBlockContext(block.Header(), api.est.blockchain, nil)
		canon       = true
	)
...
func (api *PrivateDebugAPI) computeStateDB(block *types.Block, reexec uint64) (*state.StateDB, error) {
	...
	database := state.NewDatabaseWithCache(api.est.ChainDb(), 16, "")

	for i := uint64(0); i < reexec; i++ {
		block = api.est.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
		if block == nil {
			break
		}
	...
	for block.NumberU64() < origin {
		...
		if block = api.est.blockchain.GetBlockByNumber(block.NumberU64() + 1); block == nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	tx, blockHash, _, index := rawdb.ReadTransaction(api.est.ChainDb(), hash)
	if tx == nil {
...
func (api *PrivateDebugAPI) TraceCall(ctx context.Context, args ethapi.CallArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceConfig) (interface{}, error) {
	...
	statedb, header, err := api.est.APIBackend.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	// Execute the trace
	msg := args.ToMessage(api.est.APIBackend.RPCGasCap())
	vmctx := core.NewEVMBlockContext(header, api.est.blockchain, nil)
	return api.traceTx(ctx, msg, vmctx, statedb, config)
}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewEVM(vmctx, txContext, statedb, api.est.blockchain.Config(), vm.Config{Debug: true, Tracer: tracer})
...
func (api *PrivateDebugAPI) computeTxEnv(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(signer)
		txContext := core.NewEVMTxContext(msg)
		context := core.NewEVMBlockContext(block.Header(), api.est.blockchain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		// Not yet the searched for transaction, execute on top of the current state
		vmenv := vm.NewEVM(context, txContext, statedb, api.est.blockchain.Config(), vm.Config{})
