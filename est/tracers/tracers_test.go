...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gest console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = est.getBlock(est.getTransaction(tx).blockHash);
  var genesis = est.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.est.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  est.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
