...
type downloadTester struct {
	...
	stateDb estdb.Database // Database used by the tester for syncing from peers
	peerDb  estdb.Database // Database of the peers containing all data
...
func testSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	checkProgress(t, tester.downloader, "pristine", EvolutionStellarToken.SyncProgress{})
	...
	<-starting
	checkProgress(t, tester.downloader, "initial", EvolutionStellarToken.SyncProgress{
	...
	<-starting
	checkProgress(t, tester.downloader, "completing", EvolutionStellarToken.SyncProgress{
	...
	progress <- struct{}{}
	pending.Wait()
	checkProgress(t, tester.downloader, "final", EvolutionStellarToken.SyncProgress{
		StartingBlock: uint64(chain.len()/2 - 1),
...
func checkProgress(t *testing.T, d *Downloader, stage string, want EvolutionStellarToken.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	checkProgress(t, tester.downloader, "forking", EvolutionStellarToken.SyncProgress{
