...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [est/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    estdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [est/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [est/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [est/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [est/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [est/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [est/62] Channel to feed the header processor new tasks
	...
	stateCh        chan dataPack // [est/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb estdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() EvolutionStellarToken.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "est", p.version, "head", hash, "td", td, "mode", mode)
