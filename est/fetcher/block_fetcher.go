...
var (
	blockAnnounceInMeter   = metrics.NewRegisteredMeter("est/fetcher/block/announces/in", nil)
	blockAnnounceOutTimer  = metrics.NewRegisteredTimer("est/fetcher/block/announces/out", nil)
	blockAnnounceDropMeter = metrics.NewRegisteredMeter("est/fetcher/block/announces/drop", nil)
	blockAnnounceDOSMeter  = metrics.NewRegisteredMeter("est/fetcher/block/announces/dos", nil)

	blockBroadcastInMeter   = metrics.NewRegisteredMeter("est/fetcher/block/broadcasts/in", nil)
	blockBroadcastOutTimer  = metrics.NewRegisteredTimer("est/fetcher/block/broadcasts/out", nil)
	blockBroadcastDropMeter = metrics.NewRegisteredMeter("est/fetcher/block/broadcasts/drop", nil)
	blockBroadcastDOSMeter  = metrics.NewRegisteredMeter("est/fetcher/block/broadcasts/dos", nil)

	headerFetchMeter = metrics.NewRegisteredMeter("est/fetcher/block/headers", nil)
	bodyFetchMeter   = metrics.NewRegisteredMeter("est/fetcher/block/bodies", nil)

	headerFilterInMeter  = metrics.NewRegisteredMeter("est/fetcher/block/filter/headers/in", nil)
	headerFilterOutMeter = metrics.NewRegisteredMeter("est/fetcher/block/filter/headers/out", nil)
	bodyFilterInMeter    = metrics.NewRegisteredMeter("est/fetcher/block/filter/bodies/in", nil)
	bodyFilterOutMeter   = metrics.NewRegisteredMeter("est/fetcher/block/filter/bodies/out", nil)
)
...
type BlockFetcher struct {
	...
	fetchingHook       func([]common.Hash)               // Method to call upon starting a block (est/61) or header (est/62) fetch
	completingHook     func([]common.Hash)               // Method to call upon starting a block body fetch (est/62)
	importedHook       func(*types.Header, *types.Block) // Method to call upon successful header or block import (both est/61 and est/62)
}
