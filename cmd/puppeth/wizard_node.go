...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.eststats == "" {
		log.Error("No eststats server configured")
		return
	}
	...
	if infos.eststats == "" {
		...
		infos.eststats = w.readString() + ":" + w.conf.eststats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.eststats)
		infos.eststats = w.readDefaultString(infos.eststats) + ":" + w.conf.eststats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy EvolutionStellarToken node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
