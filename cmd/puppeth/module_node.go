...
// nodeDockerfile is the Dockerfile required to run an EvolutionStellarToken node.
var nodeDockerfile = `
FROM EvolutionStellarToken/client-go:latest
...
{{end}}
RUN \
  echo 'gest --cache 512 init /genesis.json' > gest.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.EvolutionStellarToken/keystore/ && cp /signer.json /root/.EvolutionStellarToken/keystore/' >> gest.sh && \{{end}}
	echo $'exec gest --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --eststats \'{{.eststats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gest.sh

ENTRYPOINT ["/bin/sh", "gest.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an EvolutionStellarToken node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.EvolutionStellarToken{{if .Ethashdir}}
...
// deployNode deploys a new EvolutionStellarToken node container to a remote machine via SSH,
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"eststats":   config.eststats[:strings.Index(config.eststats, ":")],
		"Etherbase":  config.etherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"eststats username":        info.eststats,
	}
	if info.gasTarget > 0 {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gest --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.EvolutionStellarToken"],
		ethashdir:  infos.volumes["/root/.ethash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		eststats:   infos.envvars["STATS_NAME"],
