...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for eststats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["eststats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["eststats"] = infos.Report()
		eststats = infos.config
	}
