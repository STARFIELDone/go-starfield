...
// ethstatsDockerfile is the Dockerfile required to build an eststats backend
// and associated monitoring site.
var ethstatsDockerfile = `
FROM puppeth/eststats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ethstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an eststats monitoring site.
var ethstatsComposefile = `
version: '2'
services:
  eststats:
    build: .
    image: {{.Network}}/eststats
...
// deployEthstats deploys a new eststats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the eststats service
	if nocache {
...
// ethstatsInfos is returned from an eststats status check to allow reporting
...
// checkEthstats does a health-check against an eststats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible eststats container on the host
	...
	if err = checkPort(host, port); err != nil {
		log.Warn("eststats service seems unreachable", "server", host, "port", port, "err", err)
	}
