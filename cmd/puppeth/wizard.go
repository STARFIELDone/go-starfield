...
type config struct {
	...
	eststats  string   // eststats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // EvolutionStellarToken services known to be running on servers
...
// it to an EvolutionStellarToken address.
...
// converts it to an EvolutionStellarToken address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by eststats, which compares IPs textually, not by value.
