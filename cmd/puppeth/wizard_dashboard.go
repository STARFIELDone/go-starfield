...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"eststats", "explorer", "wallet", "faucet"} {
	...
	// If we have eststats running, ask whether to make the secret public or not
	if w.conf.eststats != "" {
		fmt.Println()
		fmt.Println("Include eststats secret on dashboard (y/n)? (default = yes)")
