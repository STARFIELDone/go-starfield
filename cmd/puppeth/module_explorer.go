...
RUN \
  echo 'gest --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gest --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --eststats \'{{.eststats}}\' --cache=512 --http --http.api "net,web3,est,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gest --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --eststats \'{{.eststats}}\' --cache=512 --http --http.api "net,web3,est,shh,debug" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        volumes:
            - {{.Datadir}}:/opt/app/.EvolutionStellarToken
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"EthPort":     config.node.port,
		"EthName":     config.node.eststats[:strings.Index(config.node.eststats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"EvolutionStellarToken listener port ": strconv.Itoa(info.node.port),
		"eststats username":       info.node.eststats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.EvolutionStellarToken"],
			port:     infos.portmap[infos.envvars["ETH_PORT"]+"/tcp"],
			eststats: infos.envvars["ETH_NAME"],
		},
