...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/cmd/utils"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the EvolutionStellarToken protocol")
	statsFlag   = flag.String("eststats", "", "eststats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for EvolutionStellarToken and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // EvolutionStellarToken address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an EvolutionStellarToken light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // EvolutionStellarToken protocol stack
	client *evolutionstellartoken.Client   // Client connection to the EvolutionStellarToken chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the EvolutionStellarToken light client protocol
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the EvolutionStellarToken service: %w", err)
	}

	// Assemble the eststats monitoring and reporting service'
	if stats != "" {
		if err := eststats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := evolutionstellartoken.NewClient(api)
...
// close terminates the EvolutionStellarToken connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the EvolutionStellarToken address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/EvolutionStellarToken/go-EvolutionStellarToken/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and EvolutionStellarToken address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// and scrape it for the EvolutionStellarToken address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No EvolutionStellarToken address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and EvolutionStellarToken address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the EvolutionStellarToken address and profile URL.
	res, err := http.Get(url)
	if err != nil {
		return "", "", common.Address{}, err
	}
...
// authNoAuth tries to interpret a faucet request as a plain EvolutionStellarToken address,
