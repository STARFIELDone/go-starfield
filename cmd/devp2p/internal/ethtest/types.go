...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/internal/utesting"
...
// Status is the network packet for the status message for est/64 and later.
type Status struct {
...
func (c *Conn) handshake(t *utesting.T) Message {
	...
	case *Hello:
		...
		if c.ethProtocolVersion == 0 {
			t.Fatalf("unexpected est protocol version")
		}
		return msg
	default:
...
// negotiateEthProtocol sets the Conn's est protocol version
// to highest advertised capability from peer
func (c *Conn) negotiateEthProtocol(caps []p2p.Cap) {
	var highestEthVersion uint
	for _, capability := range caps {
		if capability.Name != "est" {
			continue
		}
...
loop:
	...
	// make sure est protocol version is set for negotiation
	if c.ethProtocolVersion == 0 {
		t.Fatalf("est protocol version must be set in Conn")
	}
