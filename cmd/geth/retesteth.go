...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/consensus/misc"
...
func (api *RetestethAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(estdb, genesis)
	if err != nil {
		return false, err
	}
	...
	blockchain, err := core.NewBlockChain(estdb, nil, chainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
		return false, err
	}
	...
	api.db = state.NewDatabase(api.estdb)
...
func (api *RetestethAPI) mineBlock() error {
	...
	parentHash := rawdb.ReadCanonicalHash(api.estdb, number)
	parent := rawdb.ReadBlock(api.estdb, parentHash, number)
...
func (api *RetestethAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.estdb, txHash, api.chainConfig)
	if receipt == nil {
...
func retesteth(ctx *cli.Context) error {
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "est", "debug", "web3"}, srv, false)
	if err != nil {
