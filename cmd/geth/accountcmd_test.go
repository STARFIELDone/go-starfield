...
func TestAccountListEmpty(t *testing.T) {
	gest := runGeth(t, "account", "list")
	gest.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gest := runGeth(t, "account", "list", "--datadir", datadir)
	defer gest.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gest := runGeth(t, "account", "new", "--lightkdf")
	defer gest.ExpectExit()
	gest.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gest := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gest.ExpectExit()
	gest.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gest := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gest.ExpectExit()
	gest.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gest.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gest.StderrText(), m) {
...
// https://github.com/EvolutionStellarToken/go-EvolutionStellarToken/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gest.SetTemplateFunc("keypath", func(file string) string {
