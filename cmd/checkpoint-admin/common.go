...
import (
	...
	"github.com/EvolutionStellarToken/go-EvolutionStellarToken/contracts/checkpointoracle"
...
func newClient(ctx *cli.Context) *evolutionstellartoken.Client {
	client, err := evolutionstellartoken.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to EvolutionStellarToken node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, evolutionstellartoken.NewClient(client))
	if err != nil {
