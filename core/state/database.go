...
// Trie is a EvolutionStellarToken Merkle Patricia trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb estdb.KeyValueWriter) error
}

// NewDatabase creates a backing store for state. The returned database is safe for
// concurrent use, but does not retain any recent trie nodes in memory. To keep some
// historical state in memory, use the NewDatabaseWithCache constructor.
func NewDatabase(db estdb.Database) Database {
...
// large memory cache.
func NewDatabaseWithCache(db estdb.Database, cache int, journal string) Database {
