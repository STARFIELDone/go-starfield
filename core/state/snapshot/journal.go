...
func loadAndParseLegacyJournal(db estdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
...
func loadAndParseJournal(db estdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gest
	...
	// It can happen that Gest crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb estdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, error) {
	...
	//
	// Possible scenario: Gest was crashed without persisting journal and then
