...
type ChainIndexer struct {
	chainDb  estdb.Database      // Chain database to index the data from
	indexDb  estdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb estdb.Database, indexDb estdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
