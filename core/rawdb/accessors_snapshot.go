...
func ReadSnapshotRoot(db estdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db estdb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db estdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db estdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db estdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db estdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db estdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db estdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db estdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db estdb.Iteratee, accountHash common.Hash) estdb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db estdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db estdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db estdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db estdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db estdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db estdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db estdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db estdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db estdb.KeyValueWriter) {
